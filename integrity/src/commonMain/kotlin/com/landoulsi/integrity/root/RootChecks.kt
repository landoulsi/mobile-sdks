...
internal fun checkSuBinaries(context: RootCheckContext): List<IntegritySignal> {
...
internal fun checkMagisk(context: RootCheckContext): List<IntegritySignal> {
...
internal fun checkTestBuildKeys(context: RootCheckContext): IntegritySignal? {
    val buildTag = try {
        context.getBuildTag()
    } catch (_: Exception) {
        return null
...
internal fun checkWritableSystem(context: RootCheckContext): IntegritySignal? {
    val mounts = try {
        context.readFileLines("/proc/mounts")
    } catch (_: Exception) {
        return null
...
internal fun checkSuperuserApps(context: RootCheckContext): List<IntegritySignal> {
    val currentTimestampMs = Clock.System.now().toEpochMilliseconds()
